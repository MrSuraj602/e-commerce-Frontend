...
export default function Product() {
  ...
  const handleFilter = (value, sectionId) => {
    ...
    const existing = searchParam.get(sectionId);
    const filterValues = existing ? existing.split(',').filter(Boolean) : [];
    const hasValue = filterValues.includes(value);
    const nextValues = hasValue
      ? filterValues.filter((item) => item !== value)
      : [...filterValues, value];

    if (nextValues.length > 0) {
      searchParam.set(sectionId, nextValues.join(','));
    } else {
          ...
          <section aria-labelledby="products-heading" className="pt-6 pb-24">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-5">
              ...
              <form className="hidden lg:block">
                <h2 className="font-black text-xl opacity-50">Filters</h2>
                {filters.map((section) => (
