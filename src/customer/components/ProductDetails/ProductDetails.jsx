...
export default function ProductDetails() {
  ...
  const normalizedSizes = (product?.sizes || []).map((size) => ({
    ...size,
    inStock: Number(size.quantity) > 0,
  }));
  const firstInStock = normalizedSizes.find((s) => s.inStock) ?? null;
  const galleryImages = product?.imageUrl
    ? [product.imageUrl, ...products.images.slice(1)]
    : products.images;
  ...
  useEffect(() => {
    if (firstInStock && !selectedSize) {
      setSelectedSize(firstInStock.name)
    }
  ...
  if (loading && !product) {
    return <div className="px-4 py-10 text-center text-gray-600">Loading product...</div>
  }

  if (!product) {
    return <div className="px-4 py-10 text-center text-red-500">Product not found.</div>
  }
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 px-4 pt-10">
          ...
          <div className="flex flex-col items-start">
              <div className="w-full overflow-hidden rounded-lg bg-gray-50">
                <img
                  alt={product?.name}
                  src={galleryImages[selectedImageIndex] || product?.imageUrl || products.images[0].src}
              ...
              <div className="mt-4 flex gap-3">
                {galleryImages.map((item, i) => (
                  ...
                  >
                    <img alt={product?.name || 'product'} src={item} className="h-16 w-24 object-cover" />
                  </button>
                ))}
            </div>
          </div>
          ...
          <div className="mx-auto w-full max-w-2xl py-6">
            <h1 className="text-2xl font-bold tracking-tight text-gray-700 sm:text-3xl">{product?.brand}</h1>
            <h1 className="text-lg font-semibold opacity-60 tracking-tight text-gray-600 sm:text-2xl">{product?.title}</h1>
            ...
            <div className="flex space-x-5 items-center text-lg lg:text-xl text-gray-700 mt-6">
              ...
              <p className="opacity-50 line-through">₹{product?.price?.toLocaleString()}</p>
            ...
            <div className="mt-8">
              ...
              <p className="mt-4 text-sm text-gray-600">{products.details}</p>
            </div>
          </div>
        </section>
